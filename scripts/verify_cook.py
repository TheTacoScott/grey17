...
Verifies a fan-rendered cooked output against the recipe cooked_phash_sequence
by comparing pHashes frame-by-frame and reporting accuracy statistics.

The recipe must have been fingerprinted with fingerprint-cook first, which
populates cooked_phash_sequence from the author's own render.

Usage:
    python3 verify_cook.py \
        --recipe /work/recipe/edit.recipe.yaml \
        --video  /work/cooked/fan_S01E01.mkv \
        --sample-rate 24 \
        --output /work/out/verify_cook_report.csv
"""
import argparse
import csv
import os
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Compare a fan-rendered cooked output against the recipe cooked_phash_sequence.",
    )
    p.add_argument("--recipe",      required=True)
    p.add_argument("--video",       required=True, help="Path to the fan-rendered cooked output")
...
def main():
    ...
    phash_seq = recipe.get("cooked_phash_sequence")
    if not phash_seq:
        print("ERROR: recipe has no cooked_phash_sequence. Run fingerprint-cook first.",
              file=sys.stderr)
        sys.exit(1)
    ...
    duration_s = scene.get("duration_seconds") or (len(phash_seq) / fps)

    n_recipe = len(phash_seq)
    print("Recipe:   {} frames  {:.6f}fps  {:.3f}s".format(n_recipe, fps, duration_s))
    ...
    print("Extracting pHashes from fan cooked video at full fps ({:.6f})...".format(fps))
    ...
    def progress(n):
        ...
        print("  ... {} / ~{} frames  ({:02d}:{:05.2f} / {:.0f}s)".format(
            n, n_recipe, m, s, duration_s), flush=True)
    ...
    fan_hashes = all_hashes[::sample_rate]
    n_got      = len(fan_hashes)
    print("  Extracted {} frames, subsampled to {}".format(len(all_hashes), n_got))

    if n_got == 0:
        print("ERROR: no frames extracted from fan cooked video", file=sys.stderr)
        sys.exit(1)

    print("Comparing to recipe cooked_phash_sequence...")
    distances = []
    rows = []

    for i, fan_hash in enumerate(fan_hashes):
        recipe_idx = i * sample_rate
        if recipe_idx >= n_recipe:
            break
        dist = phash_distance(fan_hash, phash_seq[recipe_idx])
    ...
    if mean_dist < 3.0 and p95_dist < 6.0:
        verdict = "PASS"
    elif mean_dist < 8.0 and p95_dist < 15.0:
        verdict = "WARN"
    else:
        verdict = "FAIL"

    print()
    print("=== Verify Cook Results ({}) ===".format(verdict))
